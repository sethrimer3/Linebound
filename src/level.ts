...
export interface LevelInstance {
  ...
  /**
   * World-space spawn positions for enemy stickmen ('A' tiles).
   * Each entry is { x, y } where y is the pelvis world coordinate
   * (top of the tile — the stickman falls to the ground from there).
   */
  stickmanSpawns: Array<{ x: number; y: number }>;
...
export function parseLevel(def: LevelDef): LevelInstance {
  ...
  const stickmanSpawns: Array<{ x: number; y: number }> = [];
  ...
  for (let r = 0; r < rows; r++) {
    ...
    for (let c = 0; c < cols; c++) {
      ...
      if (ch === '#') {
        ...
        // Enemy stickman spawn — y is the top of the tile (pelvis position).
        // The stickman falls to the ground naturally from here on level start.
        stickmanSpawns.push({
          x: c * TILE_SIZE + TILE_SIZE / 2,
          y: r * TILE_SIZE,
        });
